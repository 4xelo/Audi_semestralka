...
namespace structures
{
	...
	inline void SequenceTable<K, T>::insert(const K& key, const T& data)
	{
		if (!containsKey(key))
        {
            ...
            throw std::logic_error("SequenceTable<K,T>::insert : Kluc sa v tabulke uz nachadza");
        }
	}
	...
	inline T SequenceTable<K, T>::remove(const K& key)
	{
        ...
        if (item != nullptr) {
            ...
            throw std::logic_error("SequenceTable<K, T>::remove : Kluc sa v tabulke nenachadza");
        }
	}
    ...
    inline SequenceTable<K, T>& SequenceTable<K, T>::assignSequenceTable(SequenceTable<K, T>& other)
    {
        if (this != &other) {
            clear();
            for (auto item : *other.list_) { //moze byt iba other, lebo mame naprogramovane iteratory
